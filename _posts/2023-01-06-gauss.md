...
次のような積分公式がある．ただし $a$ は正の実数で，$\bm A$ は半正定値行列としておく．
...
<span class=red>**赤字で示された箇所に同じものが入る**</span> と覚えておけばよい．左辺の正規化定数を右辺に弾き飛ばすと，よくある **ガウス積分 (Gaussian integral)** の公式の形となる．とくに2つ目のベクトル $\bm x$ に対して実行されるガウス積分のことを **多次元ガウス積分 (multidimentional Gaussian integral)** ということがある．
...
積分は被積分関数に含まれる積分変数 ($x, \bm x$ など) が取りうるすべての値に対して実行される．すなわち，
...
で与えられるから，次のように計算できる (2次形式の平方完成は [こちら](/2022/01/05))．
