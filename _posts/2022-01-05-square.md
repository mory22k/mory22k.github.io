...
以下のような変形を行うことを **平方完成 (completing the square)** という．
...
ただし $\bm A$ は正則な対称行列，$\bm x$ は変数ベクトル，$\bm b, \bm x_0$ は定数ベクトル，$c, c_0$ は定数スカラーとする．
...
> 1. まず基本の形をつくる．
> 2. 次いで左辺1次の項をヒントに括弧の中の $\textcircled{}$ を当てる．
> 3. 括弧を2乗すると何が出てくるかに着目して $\triangle$ を当てる．
