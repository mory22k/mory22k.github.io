...
が小さくなる領域では、励起状態へ遷移しないために、よりゆっくりとハミルトニアンを変化させる必要があります。断熱条件の定量的な導出や、より詳細な「十分ゆっくり」の定義については、この記事では省略し、ここではひとまず理想的な条件下での時間発展を考えます。

系を $t=0$ で $\hat{H}_0$ の基底状態に用意します。初期時刻の基底状態は、この記事の後半で導出するように、あらゆる計算基底 $\ket{\bm x}$ の重ね合わせ状態です。
...
下段では、各時刻において、各サイト $i$ のスピンの向きが $z$ 方向をどれほど向きやすいかをヒートマップで表しています。これを、その基底状態において Pauli $Z$ 演算子を測定したときの期待値 $m_i^z(t)$ で定量化しています。

$$
  m_i^z(t)
  =
  \bra{E_0(t)}\hat{\sigma}_i^z\ket{E_0(t)}
  =
  \sum_{x\in\{0,1\}^N}(-1)^{x_i}\left|\braket{x|E_0(t)}\right|^2
$$

青色は $0$ を取りやすく、赤色は $1$ を取りやすいことを示します。初期時刻では、各サイトのスピンは $x$ 方向に揃っており、$z$ 方向の磁化は $m_i^z(0)=0$ です。終時刻では、2番目のサイトのスピンが $1$ 方向に揃い、他のサイトは $0$ 方向に揃うことがわかります。

このようにして、最終的な基底状態を断熱時間発展によって導くことができれば、量子アニーリングによって最適化問題の解を得ることができる (はずな) のです。

## 初期時刻と終時刻の基底状態の導出

ここまでは数値計算の話でした。ここからは、初期時刻と終時刻の基底状態を導出してみます。系のハミルトニアンは
