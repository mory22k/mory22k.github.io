...
$\bm{x}_{\partial a}$ は、因子 $f_a$ に依存する変数の集合です。例えば、$f_2$ が $x_1$ と $x_2$ に依存する場合には、$\bm{x}_{\partial 2} = \{x_1, x_2\}$ となります。以降 $\bm{x}_{\partial a}$ は、ベクトルとして表すこともあれば、集合として表すこともあるとします。
