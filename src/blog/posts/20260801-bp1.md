...
**近似:** 近似分布 $b(\bm{x})$ が、因子周辺分布 $b_a(\bm{x}_{\partial a})$ と変数周辺分布 $b_i(x_i)$ を用いて、次の形で近似できると仮定します。
...
この仮定のもと計算される $S[b]$ を $S_\text{B}[b]$ と書くことにすると、次のように計算されます。
...
$S_\text{B}[b]$ は**Betheエントロピー**と呼ばれます。そして $S[b]$ をBetheエントロピー $S_\text{B}[b]$ で近似することを**Bethe近似**といいます。このBethe近似を導入した結果として、KLダイバージェンスは次式で近似されます。
...
ループを含む場合、そもそも、メッセージが常に収束するとは限りません。それでもメッセージが収束した場合、その固定点は、ここまでに導入したBethe近似のもとでのKLダイバージェンスの停留点に対応します。すなわち、試行分布 $b$ が木構造をもつという仮定のもとで $D_\text{KL}(b \parallel p)$ を変分した結果、という意味における近似解として解釈できます。
